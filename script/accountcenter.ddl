...
(
    ...
    business_timestamp    bigint         not null comment '业务发生时间，例如交易时间',
...
(
    ...
    accounting_timestamp       bigint       not null comment '记账时间，以账务系统时间为准',
    business_timestamp    bigint       not null comment '业务发生时间，例如交易时间，以业务传入时间为准',
    currency           varchar(3)     not null comment '币种',
    action             varchar(8)     not null comment '记账动作: IN, OUT, FREEZE, UNFREEZE, DEBIT, CREDIT',
    amount             decimal(21, 4) not null comment '金额',
    trans_seq_no       varchar(32)    not null comment '交易流水号',
...
(
    ...
    created_time      bigint      null comment '创建时间（毫秒数）',
    modified_time     bigint      null comment '修改时间（毫秒数）',
    unique key uk_balance_statistics (tenant_id, account_book, account_no, balance_type, statistics_rule,
                                      statistics_period)
)
    comment '账户余额变动统计表';
...
(
    ...
    key idx_balance (tenant_id, account_book, account_no, balance_type, business_timestamp),
